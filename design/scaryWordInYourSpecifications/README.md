...
Также был вариант сделать не в базе, а в памяти, где и рассчитывалось бы экстраполяция для сервиса со всеми скидками, с возможностью их модификации и просмотром в реальном времени, но без сохранения изменения.                                                
...
Поскольку это был первым моим опытом, я последовал совету "сделать все линейно". Со временем, в проект начали добавлять условия "Для такого сервиса нужно брать доп данные из таблицы ...", "для группы сервисов нет данных в таблице ..., 
посему нужно брать данные за иной период" и общий алгоритм "пройти в определенном порядке и применить соответствующие формулы" превратился в сложнынй алгоритм с кучей условий, которые было тяжело внедрить. 
...
Можно ещё вспомнить задачку, которая требовала добавить в сервис функциональность, которая фильтровала бы запросы с методами POST.
Казалось, что одного if-a будет достаточно, но в последствии запросы на добавление новой функциональности разрастался, когда в запросе с определенным ключом
нужно было достать данные и отправить в другой сервис, или, например, если тело запроса было пустым, то отправить запрос обратно.
По мере, условие на проверку метода POST, содержало несколько вложенных условий для каждой функциональности. Поддержка проекта оказалась затруднительной и пришли к выводу, что нужно изменить проект целом, поскольку он не соответсвовал
новым требования.

Решили проблему следующим образом: вместо того, чтобы все запросы принимать из одного ендпоинта, создали сервис, который имел для каждого случая соответствующий ендпоинт.
Это странно звучит, почему не сделали так изначально, но смотря на документацию в 5 страниц, казалось, что проект будет простым и редко изменяться, посему сделали изначально императивно.
...
Вспоминая примеры из жизни, связанные с проблемой в спецификациях, выявил для себя, что если требования не говорят о чем-то конкретном, охватывая сущности в одну охапку, 
то спроектировать гибкую систему невозомжно, поскольку нужно учитывать все возможные варианты развития событий, что априори невозможно. Конкретизация требований позволяет делать 
предположения в определенном направлении, и, на их основе проектировать систему, готовую к будущим изменениям и нововведениям. Хоть я и работал в небольших компаниях, но после расплывчатых
и неточных спецификациях, о которых мы мало что можем сказать, приходилось общаться в офисе с менеджерами, чтобы конкретизировать требования. И если в маленьких компаниях это поощрается, то вот 
в больших, вероятнее всего, это нежелательно. Конкретные спецификации важны также и со стороны разработчиков (вероятнее всего сеньоров), поскольку от них зависит выбор инструментов, подхода, парадигм.  
Подобное слово также подразумевает следующие вещи: отсутствие изолированности (пример 1), сущности неразличаемы (в контексте, что они в одной группе).
