...
func IsContainerSubnetHTTP(r *http.Request) bool {
  cameIP := r.Header.Get(xRealIP)                                            #1
  ip, parseErr := netip.ParseAddr(cameIP)                                    #2
  subnet, parsePrefixErr := netip.ParsePrefix(server.ServerCfg.Subnet)       #3
...
Казалось бы, что IsContainerSubnetHTTP нарушает SRP, т.к. и собирает данных из заголовка, и парсит  IP, но последнее логическое выражение после return явно говорит, что 
IP принадлежит посети только при выполнении всех условий, и с такой точки зрения, SRP явно удовлоряется.




# По итогу:
В моем мозгу выстроился паттерн, что, если работаю не с состоянием (в плане ячейки, для этого имеются АТД), а с иммутабельными (в плане, что изменять не планирую внутреннее состояние компонента),
то для этого почти всегда можно создать функцию, которая будет нести ответственность за определенное действие. Хорошее замечание, которое говорит, что SRP говорит об ответственности, поскольку "что делает" и "за что несет ответственность" все-таки разные вещи.

Если же говорить про классы и структуры с методами, то мне кажется, что говорить о том, удовлетворяет ли SRP структура данных можно лишь исходя из контекста. 
В качестве аргумента приведу графы. Их можно реализовать разными способами (матрица, список смежности), операции над ними могут быть разными: добавление/удаление вершин, traversal, выявление компонентов, minimum spanning-tree и прочее.
Можно возразить, что  traversal, выявление компонентов, minimum spanning-tree нужно сделать функциями, которые принимает в качестве аргумента экземпляр графа, но тогда нам нужно знать внутренее устройство графа, чтобы его "пройти".
Можно реализовать разные итераторы для каждого алгоритма, но на практике редко к одному графу применяются разные алгоритмы.
То есть, если от графа нам нужно получить только кратчайший путь, то создаем ShortestPathGraph и метод стоит ему добавить, который вернет список путем. Для других ситуаций создаем иную АТД.
С ФП, конечно, тут все намного проще :)
