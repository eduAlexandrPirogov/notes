...
К элиминации if-ов нужно подходить в то вреям, когда мы хотим улучшить код. То есть, мне кажется, что сначала надо писать "простой", "прямолинейный" код с if-ами.
Затем, по мере того, мы видим, что отступов становится довольно много, постепенно избавляться от них.

Вообще, без if-ов совсем не обойтись в следующих случаях:
1) например в Go, когда возвращаются несколько значений, а pattern matching отсутствует, то без if-ов не обойтись
2) Когда мы создаем собственные АТД из базовых типов данных. Тут важно понимать, что это фундамент, на которой и будет строиться наша система. А вот сама система, которая строится на этих блоках, должна быть без if-ов (по крайней мере, мне кажется, это более чем реально, особенно с собстевнными АТД, мапами, а если ЯП поддерживают функциии первого порядка, то вообще сказка).

В остальных случаях, это решается довольно просто.
